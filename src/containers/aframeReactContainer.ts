import { Action } from 'typescript-fsa'
import { Dispatch } from 'redux'
import { connect } from 'react-redux'
import { AppState } from '../store'
import { aframeReactAction } from '../actions/aframeReactAction'
import { aframeReactComponent } from '../components/aframeReactComponent'

export interface AframeReactAction {
    // updateName: (v: string) => Action<string>
    // updateEmail: (v: string) => Action<string>
}

function mapDispatchToProps(dispatch: Dispatch<Action<string>>) {
    return {
        // updateName: (v: string) => dispatch(hogeAction.updateName(v)),
        // updateEmail: (v: string) => dispatch(hogeAction.updateEmail(v))
    }
}

function mapStateToProps(appState: AppState) {
    return Object.assign({}, appState.hoge)
}

export default connect(mapStateToProps, mapDispatchToProps)(aframeReactComponent)